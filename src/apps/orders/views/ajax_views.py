...
def search_clients(request):
    ...
    This view requires the user to be authenticated, as it is decorated with
    @login_required(). It restricts the request method to GET only and retrieves
    clients whose phone numbers match the search query. The client data is sorted
    by their orders count in descending order and limited to a maximum of 10 results.
    If the search query (phone) is less than 3 characters, it directly returns an
    empty client list.
...
def search_products(request):
    ...
    This function allows users to search for products by name or category. It retrieves
...
def create_order_ajax(request):
    """
    Handles creating a new order via an AJAX POST request. This function validates
    ...
    try:
        ...
        for item_data in data.get("items", []):
            try:
                ...
                # Створюємо OrderItem (валідація відбудеться автоматично)
...
def update_order_ajax(request, order_id):
    ...
    try:
        ...
        if client_id:
            ...
            # Створення нового клієнта або отримання існуючого
            phone = data.get("client_phone")
            first_name = data.get("client_first_name")
...
def update_order_item(request, order_id, item_id):
    """
    Updates the quantity of a specific item in an order. This function retrieves the item
    ...
    try:
        ...
        order_data = _serialize_order(item.order)
